...
→ 사내 입력 포맷 생성
→ 제품·성분·현장·공급사 데이터 누적
→ 사업장 CAS watchlist 생성
→ 월간 또는 수동 일괄 공식 API 재조회
→ 규제정보 변경 후보 감지
→ MSDS 개정 필요/현장 게시본 교체 필요 알림
```

내부 기준표는 공식 API보다 우선하는 판단 근거가 아니다. 내부 기준표는 공식 API가 제공하지 않는 업무 보조값, 테스트 데이터, 운영 백업, 또는 회사 내부 관리 기준을 위한 보조 계층이다.

확인 큐는 “추출된 모든 성분”이 아니라, 자동화가 멈추거나 사람이 조치해야 하는 항목에만 생성한다. 예: CAS 누락, 성분명 누락, 함량 누락, 공식 API 미설정/미매칭, 영업비밀, 이전 조회 결과 대비 변경 후보.

## 목적

로컬 환경에서 MSDS PDF를 읽고, 사내 MSDS 등록 화면에 필요한 후보값을 만들며, 담당자가 원문과 공식 출처를 보면서 확인한 뒤 사내 시스템에 직접 옮겨 입력할 수 있게 돕는다.

동시에 확인된 MSDS 데이터를 제품·성분·현장·공급사 단위로 누적하여, 관리대장 자동화와 CAS No. 기반 변경감시로 이어지는 기반을 만든다.
...
3. **변경감시**: 사업장에서 사용하는 CAS No. watchlist를 만들고, 공식 출처 재조회와 변경 후보 알림으로 확장한다. 1차 MVP에서도 수동 일괄 재조회와 마지막 조회 결과 저장은 포함한다.
...
→ 공식 API 우선 조회 후 원문/공식 출처와 함께 후보값 표시
→ 담당자가 확인, 수정, 보류, 공급사 확인 필요 처리
...
8. 산안법/화관법 관련 공식 API를 우선 조회하고, 내부 기준표는 API 미제공 항목과 운영 보조값을 위한 fallback/보조 데이터로 사용한다.
9. 제품별 MSDS 개정본을 비교해 CAS 추가/삭제, 함유량 변경, 유해성·위험성 변경 후보를 보여준다.
10. 확인 완료와 사내 시스템 등록 완료를 분리해 관리한다.
11. 수동 일괄 재조회 또는 예약 실행을 통해 watchlist의 공식 API 조회 결과를 이전 결과와 비교할 수 있게 저장한다.
...
6. 1차 MVP에서 공식 DB 전체를 주기적으로 수집하거나 전체 법령을 변경감시하지 않는다. 단, 사업장 CAS watchlist에 대한 수동 일괄 재조회와 이전 결과 비교 기반은 포함한다.
...
OCR 신뢰도가 낮으면 자동추출 상태가 아니라 확인필요/OCR 신뢰도 낮음 상태로 표시한다.
```
...
작업환경측정 대상물질과 특수건강검진대상물질은 사내 입력 포맷에서는 해당 여부만 표시한다. 공식 API에서 주기값이 안정적으로 제공되지 않는 경우가 있으므로, 주기 정보는 1차 MVP 사내 입력 컬럼에 강제로 넣지 않는다.

```text
작업환경측정 대상물질: Y 또는 ○
특수건강검진대상물질: Y 또는 ○
주기 정보: 앱 내부 근거/메모 또는 향후 일정관리 보조값
```

사내 시스템 컬럼에는 주기 전용 칸이 보이지 않으므로, 사용자가 제공한 엑셀 입력값 기준으로는 개월수를 무시하고 해당 컬럼에만 표시한다. 날짜와 주기는 향후 별도 일정관리 기준이 확정되기 전까지 “공식 확정값”으로 다루지 않는다.
...
초기 조회 출처 우선순위는 다음과 같다.

- 한국환경공단 등 공공데이터 API
- KOSHA MSDS/화학물질정보 API
- K-REACH 화학물질정보처리시스템 또는 관련 공공 API
- 내부 기준표: 공식 API에 없는 보조 항목, 운영 백업, 테스트 seed

가능하면 웹 브라우저 자동 조작보다 API 또는 구조화된 데이터를 우선 사용한다.

### 4.1 내부 기준표 시드 데이터

내부 기준표는 공식 API보다 우선하지 않는다. 다만 fixture 한두 개만으로는 개발과 검증 가치가 없으므로, 1차 MVP에는 최소 seed 구조를 둔다.

사용 원칙은 다음과 같다.

- 공식 API 결과가 있으면 공식 API를 우선 반영한다.
- 내부 기준표는 API 미설정, API 장애, API 미제공 항목, 회사 내부 관리 기준을 보조한다.
- 내부 기준표로만 나온 값은 `내부 기준 확인 필요` 또는 `해당 후보`로 표시하고, 공식 확정값처럼 보이지 않게 한다.
...
- 후보값
- 상태값
- 출처 유형: MSDS, 공식 API, 내부 기준표, 사용자 입력
- 출처명
- 조회일
- 근거 문구 또는 출처 참조
- 확정되지 않은 경우 다음 조치
...
1차 MVP에서 변경감시는 완성형 외부 알림 기능이 아니라 **watchlist 기반과 수동 일괄 재조회**까지 포함한다.
...
→ 관련 제품과 현장 연결
→ 마지막 공식 출처 조회 결과 저장
→ 수동 일괄 재조회 또는 예약 실행 시 변경 후보 비교 가능
```
...
월간 자동 스케줄러와 메일/메신저 알림은 2차 기능으로 둔다. 다만 1차 데이터 모델에는 마지막 조회일, 출처, 결과 해시 또는 요약값을 저장하고, 사용자가 버튼으로 일괄 재조회해 변경 후보를 확인할 수 있게 한다.
...
6. CAS No. 기반 법적 규제 항목은 공식 API를 우선 조회하고, 내부 기준표는 보조 후보로만 반환한다.
7. fixture가 아닌 CSV seed 기반 내부 기준표로 최소 보조 매칭이 동작하되, 공식 API 결과와 출처 구분이 명확하다.
...
11. 확인필요, 공급사 확인 필요, 내부 기준 확인 필요, 등록대기 큐를 볼 수 있다.
...
4. 작업환경측정 주기와 특수건강검진 주기는 향후 일정관리에서 어떤 공식/내부 기준으로 관리할 것인가?
...
10. 내부 기준표 시드가 필요한 보조 항목의 출처와 유지보수 담당자는 누구인가?
